...
import { Users, TrendingUp, TrendingDown, Award, Filter } from "lucide-react";
...
export default function AttendanceReports() {
  ...
  const lowest = report.length > 0 ? report[report.length - 1] : null;
            ...
            <table className="w-full text-sm" data-testid="attendance-report-table">
              <thead>
                <tr className="border-b border-[#E5E7EB] bg-[#F8F9FA]">
                  {["Student", "Status", "Total Sessions", "Present", "Absent", "Attendance %"].map((h) => (
                ...
                {report.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-4 py-10 text-center text-[#8A8F98]">
                      No attendance data found for this batch.
                    </td>
                  </tr>
                ) : (
                  report.map((row) => (
